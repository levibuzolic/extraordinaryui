defmodule CinderUI.Components.Overlay do
  ...
  def tooltip(assigns) do
    assigns =
      ...
      |> assign(:content_classes, [
        "pointer-events-none absolute -top-10 left-1/2 z-50 hidden -translate-x-1/2 rounded-md bg-foreground px-3 py-1.5 text-xs text-white dark:bg-muted dark:text-foreground group-hover:block group-focus-within:block",
        assigns.content_class
      ])
