defmodule ExtraordinaryUI.InstallTaskTest do
  use ExUnit.Case, async: false

  @task "extraordinary_ui.install"

  setup do
    tmp_dir =
      Path.join(
        System.tmp_dir!(),
        "extraordinary-ui-install-test-#{System.unique_integer([:positive])}"
      )
  ...
  test "installs into project root package manager when root package.json exists", %{
    ...
    assert File.exists?(Path.join(assets, "css/extraordinary_ui.css"))
    assert File.exists?(Path.join(assets, "js/extraordinary_ui.js"))
    ...
    assert app_css =~ "@source \"../../deps/extraordinary_ui\";"
    assert app_css =~ "@import \"./extraordinary_ui.css\";"

    app_js = File.read!(Path.join(assets, "js/app.js"))
    assert app_js =~ "import { ExtraordinaryUIHooks } from \"./extraordinary_ui\""
    assert app_js =~ "Object.assign(Hooks, ExtraordinaryUIHooks)"
  end
  ...
  test "skip-existing preserves generated files", %{tmp_dir: tmp_dir} do
    ...
    File.write!(Path.join(assets, "css/app.css"), "@import \"tailwindcss\";\n")
    File.write!(Path.join(assets, "js/app.js"), "let Hooks = {}\n")
    File.write!(Path.join(assets, "css/extraordinary_ui.css"), "/* sentinel css */\n")
    File.write!(Path.join(assets, "js/extraordinary_ui.js"), "// sentinel js\n")
    ...
    assert File.read!(Path.join(assets, "css/extraordinary_ui.css")) == "/* sentinel css */\n"
    assert File.read!(Path.join(assets, "js/extraordinary_ui.js")) == "// sentinel js\n"
  end
